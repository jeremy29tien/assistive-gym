...
for seed in 0 1 2; do
  ...
  config="scratch_itch/vanilla/${var1}prefs_100epochs_10patience_001lr_001weightdecay"
  ...
  cd trex/
  python3 model.py --scratch_itch --hidden_dims 128 64 --num_comps ${var1} --num_epochs 100 --patience 10 --lr 0.01 --weight_decay 0.01 --seed $seed --reward_model_path $reward_model_path > $reward_output_path

  #RL
  echo "Performing RL..."
  cd ..
  policy_save_dir="./trained_models_reward_learning/${config}_seed${seed}"
  python3 -m assistive_gym.learn --env "ScratchItchLearnedRewardJaco-v0" --algo ppo --seed $seed --train --train-timesteps 1000000 --reward-net-path $reward_model_path --save-dir $policy_save_dir
